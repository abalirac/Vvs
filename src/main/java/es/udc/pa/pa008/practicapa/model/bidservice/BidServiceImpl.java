...
public class BidServiceImpl implements BidService{
    ...
    public Bid makeBid(Long userProfileId, Long productId, double amountI)
		...
		if (product.getEndDate().after(Calendar.getInstance())){
			try {	
	    		...
	    		if (lastBid.getUserProfile() != product.getWinner()){
	    			...
	    			Bid winnerBid = bidDao.findWinnerBid(product.getProductId(),product.getWinner().getUserProfileId());
	    		...
	    		if (userProfile == product.getWinner() && amountI <= winnerBidValue)
		    		...
		    		if (amountI > winnerBidValue){
		    			...
		    			if (amountI == winnerBidValue) {
			    			...
			    			Bid bid = new Bid(userProfile, product, amountI, Calendar.getInstance(), autoIncrement(amountI), product.getWinner());
		    		...
		    		} else throw new LowBidValueException(amountI);
	    		}	
	    		...
	    		if(amountI >= product.getStartingPrice()){
		    		...
		    		Bid bid = new Bid(userProfile, product, amountI, Calendar.getInstance(), product.getAuctionValue(), userProfile);
		    		product.setWinner(userProfile);
		    		productDao.save(product);
		    		bidDao.save(bid);
		    		return bid;
		...
		/*	
	    	try {	
	    		...
	    		if (lastBid.getUserProfile() != lastBid.getWinner()){
	    			...
	    			Bid winnerBid = bidDao.findWinnerBid(product.getProductId(),lastBid.getWinner().getUserProfileId());
	    			winnerBidValue = winnerBid.getBidValue();
	    		...
	    		if(amount > winnerBidValue){
	    			...
	    			Bid bid = new Bid(userProfile, product, amount, autoIncrement(winnerBidValue), userProfile, Calendar.getInstance());
		    		bidDao.save(bid);
		    		return bid;
		    		
	    		}else if(amount > lastBid.getAuctionValue() && amount <= winnerBidValue){
	    			
	    			if(amount == winnerBidValue) throw new LowBidValueException(amount);
	    			else{	
		    			Bid bid = new Bid(userProfile, product, amount, autoIncrement(amount), lastBid.getWinner(), Calendar.getInstance());
			    		bidDao.save(bid);
			    		return bid;
	    			}
	    		...
	    		if(amount >= product.getStartingPrice()){
		    		...
		    		Bid bid = new Bid(userProfile, product, amount, product.getStartingPrice(), userProfile, Calendar.getInstance());
		    		bidDao.save(bid);
		    		return bid;
	    ...
	    */	
		...
		}else throw new TimeExpiredException(product.getEndDate());
